...
def data_fetch(conn, edges,data,avg_degree_embeddings,hidden_channel,avg_degree_embeddings2):  ## Function for fb
    pompom = []
    n = edges.shape[1]
    ...
    for i in range(0, n):
        ...
        #     #print(f"IndexError at src: {src}, des: {des}")
        #     continue
        
        pab = conn.get(a, {}).get(b, 0)
        pba = conn.get(b, {}).get(a, 0)
        
        dung = [pab, pba]
        k = 1  # Laplacian smoothing parameter
        V = 2  # Number of possible outcomes, here it's 2 because dung has 2 elements
        
        # Apply Laplacian smoothing to each element of dung
        dung = [(x + k) / (1 + V * k) for x in dung]
        pompom.append(dung)

    #print(len(pompom))
    
    pompom = torch.tensor(pompom)
    
    if False:
            l=[]

            # If node_embeddings is available, use its size
            embedding_size = data['x'].shape[1] # if node_embeddings is not None else 8  # Default to 8 if size is unknown

            default_tensor = torch.zeros(embedding_size)
            ...
            for i in range(0, n):
                ...
                embab = avg_degree_embeddings.get(a, {}).get(b, default_tensor)
                embba = avg_degree_embeddings.get(b, {}).get(a, default_tensor)

                #print(embba.shape,embab.shape)
                
                embd=torch.cat((embab,embba), dim=-1)
            ...
            #print(l.shape,pompom.shape)

            pompom=torch.cat((pompom,l), dim=1)
    ...
    if True:
            ...
            # If node_embeddings is available, use its size
            embedding_size = hidden_channel # if node_embeddings is not None else 8  # Default to 8 if size is unknown
            ...
            for i in range(0, n):
                ...
                embab = avg_degree_embeddings2.get(a, {}).get(b, default_tensor)
                embba = avg_degree_embeddings2.get(b, {}).get(a, default_tensor)
...
def data_fetch_fb(df, data_dict, edges):  ## Function for fb
    pompom = []
    n = edges.shape[1]
    ...
    for i in range(0, n):
        src = int(edges[0][i])
        des = int(edges[1][i])
        a = df['page_type'].iloc[src]
        b = df['page_type'].iloc[des]
        # except IndexError as e:
        #     #print(f"IndexError at src: {src}, des: {des}")
        #     continue
        
        pab = data_dict.get(a, {}).get(b, 0)
        pba = data_dict.get(b, {}).get(a, 0)
...
def get_class_intersection_prob(y, split_edge):
    ...
    #python main_ncn_CoraCiteseerPubmed.py  --dataset DBLP --predictor incn1cn1   --gnnlr 0.001 --prelr 0.001 --l2 0  --predp 0.3 --gnndp 0.3 --mplayers 3 --nnlayers 3   --hiddim 256 --epochs 9999 --eval_steps 5 --kill_cnt 10 --batch_size 1024 --xdp 0.3 --tdp 0.0 --pt 0.5 --gnnedp 0.0 --preedp 0.0   --probscale 5.3 --proboffset 0.5 --alpha 0.3 --ln --lnnn   --model puregcn  --testbs 8192  --maskinput  --jk  --use_xlin  --tailact --addon
    #y=y[0]
...
def loaddataset(name, use_valedges_as_input, load=None,k=0):
    ...
    return data, split_edge #,connection_probs,avg_degree_embeddings,degrees
...
def train(model,
          ...
          alpha: float=None,addon= False,cluster=False,conn=False,avg_degree_embeddings=False,avg_degree_embeddings2={},hidden_channel:int=0,df=None,data_dict={},data_name=None):
    ...
    for perm in PermIterator(
        ...
        if addon:
            ...
            #additional_features=data_fetch(conn,edge,data,avg_degree_embeddings,hidden_channel,avg_degree_embeddings2)
                ...
                additional_features=data_fetch_fb(df,data_dict,edge)
...
def test(model, predictor, data, split_edge,  evaluator_hit, evaluator_mrr, batch_size,
         use_valedges_as_input,addon,cluster,df=None,data_dict={}):#conn,avg_degree_embeddings,hidden_channel,avg_degree_embeddings2):
    model.eval()
    predictor.eval()
    ...
    pos_train_pred = torch.cat([
        ...
        for perm in PermIterator(pos_train_edge.device,
                               ...
                               dim=0)
    '''
    #additional_features=data_fetch_twitch(df, data_dict, pos_train_edge[perm].t()).to(data.x.device)
    ...
    pos_valid_pred = torch.cat([
        predictor(h, adj, pos_valid_edge[perm].t(),
                  additional_features=(data_fetch_prob(data.y, data_dict, pos_valid_edge[perm].t()).to(data.x.device) if cluster else data_fetch_fb(df, data_dict, pos_valid_edge[perm].t()).to(data.x.device)) if addon else None
                  ).squeeze().cpu()
    ...
    # pos_valid_pred = torch.cat([
    #     predictor(h, adj, pos_valid_edge[perm].t(),
    #     additional_features= (data_fetch_fb(data.y, data_dict, pos_valid_edge[perm].t()).to(data.x.device) if cluster else data_fetch_prob(df, data_dict, pos_valid_edge[perm].t()).to(data.x.device)) if addon else None
    #     ).squeeze().cpu()
    #     for perm in PermIterator(pos_valid_edge.device,
    #                              pos_valid_edge.shape[0], batch_size, False)
    # ],
    ...
    neg_valid_pred = torch.cat([
        predictor(h, adj, neg_valid_edge[perm].t(),
         additional_features=(data_fetch_prob(data.y, data_dict, neg_valid_edge[perm].t()).to(data.x.device) if cluster else data_fetch_fb(df, data_dict, neg_valid_edge[perm].t()).to(data.x.device)) if addon else None
    ...
    pos_test_pred = torch.cat([
        predictor(h, adj, pos_test_edge[perm].t(),
         additional_features=(data_fetch_prob(data.y, data_dict, pos_test_edge[perm].t()).to(data.x.device) if cluster else data_fetch_fb(df, data_dict, pos_test_edge[perm].t()).to(data.x.device)) if addon else None
    ...
    neg_test_pred = torch.cat([
       ...
       additional_features=(data_fetch_prob(data.y, data_dict, neg_test_edge[perm].t()).to(data.x.device) if cluster else data_fetch_fb(df, data_dict, neg_test_edge[perm].t()).to(data.x.device)) if addon else None
        ).squeeze().cpu()
...
def parseargs():
    ...
    #Underlying parameters are important
...
#        print("Shape of data.y (labels):", data.y.shape)
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            loss = train(model, predictor, data, split_edge, optimizer,
                         args.batch_size, args.maskinput, [], alpha,addon=args.addon,cluster=args.cluster,df=df,data_dict=data_dict,data_name=args.dataset)#,conn=connection_probs,avg_degree_embeddings=avg_degree_embeddings,hidden_channel=args.hiddim)
            ...
            if epoch % args.eval_steps == 0:
                results, score_emb = test(model, predictor, data, split_edge,  evaluator_hit, evaluator_mrr,
                                args.testbs, args.use_valedges_as_input,addon=args.addon,cluster=args.cluster,df=df,data_dict=data_dict)#conn=connection_probs,avg_degree_embeddings=avg_degree_embeddings,hidden_channel=args.hiddim,avg_degree_embeddings2=avg_degree_embeddings2)
