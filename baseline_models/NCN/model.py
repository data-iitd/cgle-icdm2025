...
class CNLinkPredictor(nn.Module):
    ...
    def __init__(self,
        ...
        self.lin2 = nn.Sequential(nn.LazyLinear(hidden_channels),
                                 lnfn(hidden_channels, ln),
                                 nn.Dropout(dropout, inplace=True),
                                 nn.ReLU(inplace=True),
                                 nn.Linear(hidden_channels, hidden_channels) if twolayerlin else nn.Identity(),
    ...
    def multidomainforward(self,
                           ...
                           tar_ei,
                           filled1: bool = False,
                           cndropprobs: Iterable[float] = [],additional=None):
        adj = self.dropadj(adj)
        xi = x[tar_ei[0]]
        xj = x[tar_ei[1]]
        x = x + self.xlin(x)
...
class IncompleteCN1Predictor(CNLinkPredictor):
    ...
    def multidomainforward(self,
                           ...
                           filled1: bool = False,
                           cndropprobs: Iterable[float] = [],
                           depth: int=None,additional=None):
        assert len(cndropprobs) == 0
        if depth is None:
            depth = self.depth
        adj = self.dropadj(adj)
    ...
    def forward(self,
                ...
                tar_ei,
                filled1: bool = False,
                depth: int = None,additional_features=None):
        if depth is None:
            depth = self.depth
        return self.multidomainforward(x, adj, tar_ei, filled1, [],
                                       depth,additional=additional_features)
